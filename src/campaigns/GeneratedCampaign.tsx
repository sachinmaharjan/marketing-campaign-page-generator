...
function getThemeForHoliday(name: string) {
  ...
  if (norm.includes('christmas') || norm.includes('yule') || norm.includes('boxing')) {
    return {
      bg: "bg-[#FFF5F5]",
  ...
  if (norm.includes('valentine') || norm.includes('women') || norm.includes('mother')) {
    return {
      bg: "bg-[#FFF5F8]",
  ...
  if (norm.includes('patrick') || norm.includes('earth') || norm.includes('environment')) {
    return {
      bg: "bg-[#F0FFF4]",
  ...
  if (norm.includes('diwali') || norm.includes('autumn') || norm.includes('thanksgiving')) {
    return {
      bg: "bg-[#FFFBEB]",
  ...
  // Default (similar to ChildrensDay)
  return {
    bg: "bg-[#FFF9F0]",
...
export default function GeneratedCampaign({
  ...
  const theme = getThemeForHoliday(holidayName);
  const images = getImagesForHoliday(holidayName);
  
  return (
    <div className={`min-h-full ${theme.bg} font-sans overflow-x-hidden selection:bg-black/10`}>
      {/* Decorative blobs */}
      ...
      <footer className={`text-center py-10 ${theme.textMuted} font-medium pb-24`}>
